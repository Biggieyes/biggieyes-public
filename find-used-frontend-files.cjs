// Skript pro výpis používaných frontendových souborů ve složce src/
// Uloženo jako find-used-frontend-files.cjs pro CommonJS režim
const fs = require("fs");
...
const usedFiles = allFiles.filter(f => used.has(f));
console.log("Používané soubory ve src/:");
usedFiles.forEach(f => console.log(path.relative(SRC_DIR, f)));
