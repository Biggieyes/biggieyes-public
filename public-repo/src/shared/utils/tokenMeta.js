...
export async function refreshTokenMeta({
  ...
  try {
    const ts = await biggi.totalSupply().catch(() => null);
  ...
  setBiggiData((prev) => ({
    ...
    token: {
      ...
      cap: prev.token?.cap ?? null,
      remainingMintable: prev.token?.remainingMintable ?? null,
      reserveAddr: prev.token?.reserveAddr ?? null,
      dexRecipientAddr: prev.token?.dexRecipientAddr ?? null,
      tokenREWARDSAddr: prev.token?.tokenREWARDSAddr ?? null,
      REWARDSOperator: prev.token?.REWARDSOperator ?? null,
      distributed: prev.token?.distributed ?? null,
    },
  }));
  ...
  setBiggiData((prev) => ({
    ...
    token: {
      ...
      reserveAddr: dist?.reserve || prev.token.reserveAddr,
      ...
      tokenREWARDSAddr:
        tr?.tokenREWARDS || prev.token.tokenREWARDSAddr || null,
    },
    router: {
      ...prev.router,
    ...
    reserve: {
      ...
      reserveAddress:
        res?.reserve ||
        prev.reserve?.reserveAddress ||
        prev.token?.reserveAddr ||
        null,
