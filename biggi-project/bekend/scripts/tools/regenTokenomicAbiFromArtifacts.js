...
const baseAbiDir = path.join(__dirname, '..', '..', 'contracts', 'default_workspace (10)', 'contracts', 'BIGGI_MASTER', 'TOKENOMICMAINNET', 'ABI');
const indexFile = path.join(baseAbiDir, 'index.json');

if (!fs.existsSync(indexFile)) {
  console.error('index.json not found in TOKENOMIC ABI dir:', indexFile);
  process.exit(1);
}
...
const outReport = path.join(__dirname, 'reports', 'regenTokenomicAbi.report.json');
