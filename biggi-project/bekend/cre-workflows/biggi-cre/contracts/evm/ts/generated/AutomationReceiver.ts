...
export class AutomationReceiver {
  ...
  getConsumerGasLimit(
    ...
    const callData = encodeFunctionData({
      ...
      functionName: 'getConsumerGasLimit' as const,
  ...
  writeReportFromSetConsumerGasLimit(
    runtime: Runtime<unknown>,
    target: `0x${string}`,
    selector: `0x${string}`,
    gasLimit: bigint,
    ...
    const callData = encodeFunctionData({
      abi: AutomationReceiverABI,
      functionName: 'setConsumerGasLimit' as const,
      args: [target, selector, gasLimit],
    })
  ...
  /**
   * Creates a log trigger for ConsumerGasLimitSet events.
   * The returned trigger's adapt method decodes the raw log into ConsumerGasLimitSetDecoded,
   * so the handler receives typed event data directly.
   * When multiple filters are provided, topic values are merged with OR semantics (match any).
   */
  logTriggerConsumerGasLimitSet(
    filters?: ConsumerGasLimitSetTopics[],
  ) {
    let topics: { values: string[] }[]
    if (!filters || filters.length === 0) {
      const encoded = encodeEventTopics({
        abi: AutomationReceiverABI,
        eventName: 'ConsumerGasLimitSet' as const,
      })
      topics = encoded.map((t) => ({ values: encodeTopicValue(t) }))
    } else if (filters.length === 1) {
      const f = filters[0]
    ...
    return {
      ...
      outputSchema: () => baseTrigger.outputSchema(),
      configAsAny: () => baseTrigger.configAsAny(),
      adapt: (rawOutput: EVMLog): DecodedLog<ConsumerGasLimitSetDecoded> => contract.decodeConsumerGasLimitSet(rawOutput),
    }
  }

  /**
   * Decodes a log into ConsumerGasLimitSet data, preserving all log metadata.
   */
  decodeConsumerGasLimitSet(log: EVMLog): DecodedLog<ConsumerGasLimitSetDecoded> {
    const decoded = decodeEventLog({
      abi: AutomationReceiverABI,
      data: bytesToHex(log.data),
      topics: log.topics.map((t) => bytesToHex(t)) as [Hex, ...Hex[]],
