    ...
    ReentrancyGuard
{
    ...
    // NOVÉ: procenta trvalého zdražení dle barvy pozadí (5–10 %)
    ...
    // === CENOVÁ POMOCNÁ FUNKCE – CROSS-BOOST ===
    /// @dev Cross-boost: pozadí `bg` trvale navyšuje blok se stejnou barvou (blockIdx == bg).
    ///      Final price pro dané NFT se počítá z ceny jeho bloku `blk` + jednorázového bonusu dle `bg`.
    /// @param blk  Blok NFT, do kterého spadl mint (1..10).
    /// @param bg   Kód pozadí/„barvy očí“ (1..10); trvale zvedá blockInfos[bg-1].
    /// @return baseForFinal  cena bloku `blk` v okamžiku mintu (před jednorázovým bonusem)
    /// @return finalPrice    baseForFinal navýšený o jednorázový bonus dle pozadí
    function _applyPricing(uint16 blk, uint16 bg)
        private
        returns (uint256 baseForFinal, uint256 finalPrice)
    {
        // 1) TRVALÉ NAVÝŠENÍ: „stejná barva“ – pozadí #k → zvýší blockInfos[k-1]
        BiggiPriceMathLib.BlockInfo storage biColor = blockInfos[bg - 1];
        uint256 currColor = biColor.currentPrice;
        uint8 incPct = blockBgIncreasePct[bg - 1]; // 5–10 %
        biColor.currentPrice = currColor + (currColor * incPct) / 100;

        // 2) JEDNORÁZOVÝ BONUS pro právě mintnuté NFT – odvíjí se od ceny jeho bloku `blk`
        uint256 blkPriceNow = blockInfos[blk - 1].currentPrice;
        uint8 oneOffPct = backgroundBonuses[bg - 1]; // 5–50 %
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        for (uint256 i = 0; i < MAX_SUPPLY; ++i) {
            if (!nftInfo[index].minted) {
                ...
                nftInfo[index].blockPrice  = baseForFinal; // aktuální cena bloku v okamžiku mintu
