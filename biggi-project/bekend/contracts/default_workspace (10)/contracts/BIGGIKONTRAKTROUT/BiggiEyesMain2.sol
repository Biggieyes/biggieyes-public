...
interface IBiggiRewardsPool {
    function receiveMintShare() external payable;
}
/* Likviditní pool kontrakt přijímající 18% z mintu */
interface IBiggiLiquidityPool {
    function receiveMintShare() external payable;
}
...
contract BiggiEyesMain is ERC721, Ownable, Pausable, ReentrancyGuard {
    ...
    IBiggiRewardsPool public rewardsPool;
    ...
    /* ---- Rozdělení ETH: 22% rewards, 18% liquidity ---- */
    address payable public liquiditySink;
    uint256 public liquidityBps = 1800; // 18%
    uint256 public rewardsBps   = 2200; // 22%
    ...
    /* ---- Likviditní pool kontrakt pro 18% příjem ---- */
    IBiggiLiquidityPool public liquidityPool;
    ...
    function setModules(address compute_, address vrfRouter_, address rewardsPool_) external onlyOwner {
        ...
        if (rewardsPool_ != address(0)) { rewardsPool = IBiggiRewardsPool(rewardsPool_); emit RewardsPoolSet(rewardsPool_); }
    }
    ...
    function setLiquiditySink(address payable sink, uint256 bps) external onlyOwner {
        ...
        if (tokenSink == address(0)) { tokenSink = sink; emit TokenSinkUpdated(sink, tokenSinkBps); }
    }

    function setLiquidityPool(address pool) external onlyOwner {
        liquidityPool = IBiggiLiquidityPool(pool);
    ...
    // UI používá getMintData(index) → vrátíme (ticketPrice, blockPrice, finalPrice) pro daný index
    ...
    function mintTicket() external payable nonReentrant whenNotPaused {
        ...
        _splitAndForward(); // interní volání bez {value: ...}
    ...
    function fulfillRandomFromRouter(uint256 requestId, uint256 randomWord) external {
        ...
        for (uint256 i = 0; i < MAX_SUPPLY; ++i) {
            if (!nftInfo[idx].minted) {
                ...
                // statistiky (short scope / unchecked)
                unchecked {
                    blockMintCounts[blk_ - 1]++;
                    backgroundMintCounts[bg_ - 1]++;
                }

                // 1) trvalé navýšení ceny barvy pozadí
                {
                ...
                // 2) jednorázový bonus pro právě mintnuté NFT
                uint256 blkPriceNow;
                uint256 finalPrice;
                {
            ...
            // wrap-around hledání volného indexu
    ...
    function _splitAndForward() internal {
        ...
        // 22% → RewardsPool
        uint256 toRewards = (amount * rewardsBps) / 10000;
        if (address(rewardsPool) != address(0) && toRewards > 0) {
            (bool ok1, ) = address(rewardsPool).call{value: toRewards}("");
            require(ok1, "rewards fwd failed");
        }

        // 18% → Likvidita: preferuj kontrakt liquidityPool.receiveMintShare(), jinak fallback na adresu
        uint256 toLiq = 0;
        if (liquidityBps > 0) {
            toLiq = (amount * liquidityBps) / 10000;
            if (address(liquidityPool) != address(0)) {
                (bool ok2, ) = address(liquidityPool).call{value: toLiq}(
                    abi.encodeWithSelector(IBiggiLiquidityPool.receiveMintShare.selector)
                );
                require(ok2, "liq fwd failed");
            } else if (liquiditySink != address(0)) {
                Address.sendValue(liquiditySink, toLiq);
            }
        }

        // zbytek → dev
        uint256 devPart = amount - toRewards - toLiq;
