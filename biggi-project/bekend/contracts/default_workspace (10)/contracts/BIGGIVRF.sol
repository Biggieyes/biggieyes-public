...
import "./BiggiRedeemHelper.sol";
import "./BiggiIndexHelper.sol";
    ...
    ReentrancyGuard
{
    ...
    using BiggiIndexHelper for mapping(uint256 => BiggiIndexHelper.NFTInfo);
    ...
    mapping(uint256 => BiggiIndexHelper.NFTInfo) public nftInfo;
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        if (
            tokenId >= BiggiIdHelper.BIGGI_OFFSET &&
            tokenId < BiggiIdHelper.CHARACTER_OFFSET
        ) {
            uint256 idx = BiggiIdHelper.nftIndexFromTokenId(tokenId);
            BiggiIndexHelper.NFTInfo memory info = nftInfo[idx];
    ...
    // --- Batch mint vstupenek ---
    function mintTickets(uint256 count) public payable nonReentrant whenNotPaused {
        require(count >= 1 && count <= 10, "Batch=1..10");
        require(ticketMinted + count <= MAX_TICKETS, "All tickets minted");
        require(ticketCount[msg.sender] + count <= 10, "Max 10 tickets per wallet");
    ...
    function redeemTicketAndMintNFT(uint256 ticketId) external nonReentrant whenNotPaused {
        ...
        BiggiRedeemHelper.setPendingRequest(pendingMintRequest, pendingMinters, msg.sender, requestId);
    ...
    function publicMint(uint256 index) external payable nonReentrant whenNotPaused {
        ...
        BiggiIndexHelper.NFTInfo storage info = nftInfo[index];
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        for (uint256 i = 0; i < MAX_SUPPLY; ++i) {
            if (!nftInfo[index].minted) {
                ...
                BiggiRedeemHelper.clearPendingRequest(pendingMintRequest, pendingMinters, minter, requestId);
                return;
            }
    ...
    function batchSetNFTBackgroundAndBlock(
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            BiggiIndexHelper.NFTInfo storage info = nftInfo[idx];
