    ...
    ReentrancyGuard
{
    ...
    function redeemTicketAndMintNFT(uint256 ticketId) external nonReentrant whenNotPaused {
        ...
        // ✅ Povolíme redeem, pokud existuje alespoň JEDEN nakonfigurovaný a dosud nemintnutý index.
        if (!_hasAnyConfiguredLeft()) revert NotFullyConfigured();
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        for (uint256 i = 0; i < MAX_SUPPLY; ++i) {
            // ✅ Mintujeme pouze na nakonfigurované a dosud nemintnuté indexy
            if (!nftInfo[index].minted && _isConfigured(nftInfo[index])) {
        ...
        // ✅ nedostupné vhodné indexy – konfigurace není kompletní
        revert NotFullyConfigured();
    }
    ...
    // ✅ Nové: stačí, aby existoval alespoň jeden nenamintnutý a korektně nakonfigurovaný index
