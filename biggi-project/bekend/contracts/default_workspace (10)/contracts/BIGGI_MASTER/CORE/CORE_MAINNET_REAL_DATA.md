...
Live read-only preflight after the CollectionRewards migration returned `okForDeployOnly=true`, `okForPublicLaunch=false`, 13 blockers and 2 warnings. The stricter manifest audit additionally checks LiquidityManager stored `5 POL / 5 POL` trigger/request values, auto-buyback state, all five CRE call/role bindings, and explicit Chapter 1 activation. The paused BuybackUpkeepProxy still has the erroneous `1 wei` threshold, the pair is empty, Chapter 1 Public and the receiver are paused, workflow identity is unset, and the liquidity orchestrator/keeper are paused. The warnings confirm that Chapter 1 CollectionRewards claims remain locked while its isolated `47000 POL` budget fills and that BIGGI-paid mints do not add native POL. Chapter 1 Public metadata are complete; missing final image URIs for chapters 2-5 block only those future chapters.
