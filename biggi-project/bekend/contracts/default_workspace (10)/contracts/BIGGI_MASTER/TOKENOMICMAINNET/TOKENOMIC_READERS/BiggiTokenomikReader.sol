...
contract BiggiTokenomikReader {
    ...
    struct DistributorStatus {
        ...
        uint256 pendingCollectionRewards;
        uint256 pendingReserve;
        uint256 pendingTreasury;
        uint256 pendingCommunityCenter;
    ...
    function _distributorStatus() internal view returns (DistributorStatus memory d) {
        ...
        address buybackTarget = d.buybackAgent == address(0) ? BUYBACK : d.buybackAgent;
        if (buybackTarget != address(0)) {
            try mc.pending(buybackTarget) returns (uint256 p) { d.pendingBuyback = p; } catch {}
        }
        if (d.collectionRewards != address(0)) {
            try mc.pending(d.collectionRewards) returns (uint256 p) { d.pendingCollectionRewards = p; } catch {}
        }
        if (d.reserve != address(0)) {
            try mc.pending(d.reserve) returns (uint256 p) { d.pendingReserve = p; } catch {}
        }
        if (d.treasury != address(0)) {
            try mc.pending(d.treasury) returns (uint256 p) { d.pendingTreasury = p; } catch {}
        }
        if (d.communityCenter != address(0)) {
            try mc.pending(d.communityCenter) returns (uint256 p) { d.pendingCommunityCenter = p; } catch {}
        }
    }
    ...
    function _tokenRewardsStatus() internal view returns (TokenRewardsStatus memory t) {
        ...
        try tr.emissionController() returns (address ec) { t.emissionController = ec; } catch {}
        try tr.emissionControllerEnabled() returns (bool enabled) { t.emissionControllerEnabled = enabled; } catch {}
