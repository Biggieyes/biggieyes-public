...
contract LiquidityAutomation is Ownable, ReentrancyGuard {
    ...
    uint256 public minPolWei;     // lower bound per run
    uint256 public maxPolWei;     // upper bound per run
    uint256 public minIntervalSec;  // min seconds between runs
    uint256 public lastUpkeepTime;  // timestamp of last successful perform

    event LimitsSet(uint256 minPolWei, uint256 maxPolWei);
    event IntervalSet(uint256 minIntervalSec);
    event LMSet(address lm);
    event UpkeepPerformed(uint256 requestedPol, address caller);
    ...
    constructor(
        ...
        require(minPolWei_ > 0, "min0");
        require(maxPolWei_ >= minPolWei_, "max<min");
        require(minIntervalSec_ > 0, "interval0");

        lm = ILiquidityManager(lm_);
        biggi = IERC20(biggi_);
        minPolWei = minPolWei_;
        maxPolWei = maxPolWei_;
    ...
    function _computeRequested() internal view returns (bool ok, uint256 amount) {
        ...
        // native available on reserve (prefer getPolAvailable, fallback to balance)
        uint256 availableNative = _safeGetPol(reserveAddr);
        if (availableNative < minPolWei) {
            return (false, 0);
        }

        uint256 requested = availableNative;
        if (requested > maxPolWei) requested = maxPolWei;
        if (requested < minPolWei) {
            return (false, 0);
        }
    ...
    function _safeGetPol(address reserveAddr) internal view returns (uint256) {
        uint256 available = address(reserveAddr).balance;
        try IReserveV4View(reserveAddr).getPolAvailable() returns (uint256 m) {
            if (m > 0) available = m;
        } catch {}
        return available;
    }
