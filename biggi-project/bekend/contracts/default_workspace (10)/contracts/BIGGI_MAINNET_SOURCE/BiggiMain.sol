...
// NOVÉ: interface pro Reserve hook (Main zavolá notifyBiggiReceived po přeposlání tokenů)
...
contract BiggiEyesMain is ERC721, Ownable, Pausable, ReentrancyGuard {
    ...
    /* ======= NOVÉ: owner-only settery (přidáno přesně podle požadavku) ======= */
    ...
    /// @notice Přepíše současnou cenu ticketu (owner-only)
    ...
    /// @notice Přepíše parametr zvýšení ceny po každém mintu (bps-like)
    ...
    /// @notice Nastaví currentPrice pro daný block (1..10) (owner-only)
    ...
    function redeemTicketAndMintNFT(uint256 ticketId) external nonReentrant whenNotPaused {
        ...
        pendingTicketPrice[reqId] = ticketPrice;
    ...
    function _collectBiggi(address from, uint256 amount) internal {
        ...
        if (toReserve > 0) {
            ...
            try IBiggiReserve(reserveAddress).notifyBiggiReceived(toReserve) {
                ...
                // ticho — tokeny jsou i tak na reserve adrese
            }
        }
    ...
    /* === SPLIT: 60% Distributor + 40% DEV (fix) === */
