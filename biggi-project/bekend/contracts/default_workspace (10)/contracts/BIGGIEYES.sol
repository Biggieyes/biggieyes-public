...
/// @dev Změny oproti původní verzi:
///      1) Dynamická cena bloku se zvyšuje podle MINTNUTÉHO BACKGROUNDU napříč kolekcí.
///         Při mintu s bg=X se zvýší cena bloku X o jeho procento.
///      2) Base ceny, blokové nárůsty i background bonusy jsou natvrdo v kódu (bez setterů).
///      3) findTicket() prohledává reálný rozsah ticket tokenId (TICKET_OFFSET..+MAX_TICKETS-1).
///      4) Rewards i VRF logika nedotčena.
    ...
    ReentrancyGuard
{
    ...
    // Uživatel platí pouze ticket price (s vlastním jemným růstem).
    uint256 public ticketPrice = 0.001 ether;
    uint256 public priceIncreasePerMint = 10033; // +0.33% (1.0033 * 1e4)

    // --- NOVÁ logika narůstání cen (ekonomická metrika) ---
    // Base ceny bloků (1..10) – v ether (wei). Zadání: 100M..1000M.
    ...
    // Multiplikátory nárůstu ceny při TRIGGERU přes background mint (setiny %, 10500 = +5.00 %)
    // Index 0..9 odpovídá blokům/backgroundům 1..10: Orange, Black, White, Brown, Blue, Green, Violet, Red, Pink, Rainbow
    ...
    // Jednorázové bonusy do finalPrice podle backgroundu 1..10 – v %
    uint8[10] public backgroundBonuses = [
        5, 10, 15, 20, 25, 30, 35, 40, 45, 50
    ];

    // Počítadla mintů pro BLOK (dle blockIdx NFT) + runtime struktury pro cenu (dle bloků 1..10)
    ...
    // --- Rewards fond a částky (beze změny) ---
    ...
    mapping(uint256 => bool) public orangeMainIdPaid; // Orange mainId (1..10) jen 1× globálně
    mapping(uint16 => bool) public blockPaid;         // bloky 1..9 jen 1× globálně
    mapping(address => mapping(uint16 => bool)) public userClaimedBlock; // uživatel stejný blok ne 2×
    ...
    // --- Settery URI & parametry (ponecháno – netýkají se cen bloků) ---
    ...
    // === KONSTRUKTOR S NATVRDO NASTAVENOU LOGIKOU NÁRŮSTŮ ===
    constructor(
    ...
    function publicMint(uint256 index) external payable nonReentrant whenNotPaused {
        ...
        // počítadlo mintů pro BLOK (dle blockIdx NFT)
        blockMintCounts[info.blockIdx - 1]++;

        // === Klíčové: dynamická cena se zvyšuje podle BACKGROUNDU napříč kolekcí ===
        // Trigger: background barva zvyšuje cenu stejného bloku (ekonomická metrika).
        uint16 bgIdx0 = info.background - 1;
        blockInfos[bgIdx0].updatePrice();

        // Ekonomická hodnota NFT: vychází z jeho BLOKU (nikoliv z bg triggeru)
        uint16 blkIdx0 = info.blockIdx - 1;
        uint256 currentBlockPrice = blockInfos[blkIdx0].currentPrice;

        info.ticketPrice = ticketPrice;                // co uživatel skutečně zaplatil
        info.blockPrice  = currentBlockPrice;          // aktuální cena bloku (ekonomická metrika)
        uint8 bonus      = backgroundBonuses[info.background - 1];
        info.finalPrice  = currentBlockPrice + ((currentBlockPrice * bonus) / 100); // jednorázový bonus

        emit MintFulfilled(msg.sender, tokenId, index);
    }

    // --- VRF callback (BEZE ZMĚN) ---
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        for (uint256 i = 0; i < MAX_SUPPLY; ++i) {
            if (!nftInfo[index].minted) {
                ...
                uint16 blk = nftInfo[index].blockIdx;   // eye-color block pro NFT
                uint16 bg  = nftInfo[index].background; // background barva (spouští růst)

                // počítadlo mintů pro BLOK NFT
                blockMintCounts[blk - 1]++;

                // === Dynamická cena dle BACKGROUNDU (globální trigger) ===
                uint16 bgIdx0 = bg - 1;
                blockInfos[bgIdx0].updatePrice();

                // ekonomická hodnota tohoto NFT je z jeho BLOKU
                ...
                // bonus dle backgroundu (jednorázově)
    ...
    /// @dev Prohledává reálný rozsah ticketId [TICKET_OFFSET .. TICKET_OFFSET+MAX_TICKETS-1]
    ...
    // --- Claimování peněžních odměn (BEZE ZMĚNY) ---
    ...
    /// @notice Block: drž všech 10 různých mainId (1..10) v jednom z bloků 1..9 (RAINBOW=10 vyloučen).
