 ...
 * - ERC20 BIGGI s CAP 10,000,000 BIGGI (18 dec.)
 ...
 *     2) addLiquidityFromBalance() – další doplňování LP (swap půlky nativní měny → BIGGI + přidání LP).
 *
 * DOPLNĚNO PRO FRONTEND:
 ...
 *
 * NOVÉ PO-NASAZENÍ NASTAVITELNÉ PARAMETRY (setter-y):
 *  - setRouter(address)                      – změna routeru (a wrappedNative z něj)
 *  - setLiquidityRecipient(address)         – příjemce LP tokenů (default = owner())
 *  - setLpUseBalanceBps(uint256)            – kolik % nativního zůstatku používat
 *  - setSwapPath(address[])                 – vlastní swap path (WNATIVE → ... → BIGGI)
 *  - clearSwapPath()                        – smazání custom path (zpět na [WNATIVE,BIGGI])
 *  - setSwapSlippageBps(uint256)            – tolerovaná slippage pro swap (přes getAmountsOut)
 *  - setLpAddSlippageBps(uint256)           – min. tolerance při addLiquidityETH (obě strany)
 *  - setTxDeadlineSeconds(uint256)          – deadline v sekundách pro router operace (default 600)
 *
 * NOVĚ: Router lze nastavit až PO deployi (constructor akceptuje router_ == address(0)).
 */
...
interface IUniswapV2Router02 {
    ...
    // view quoting – používáme pro výpočet amountOutMin (slippage)
    function getAmountsOut(uint amountIn, address[] calldata path) external view returns (uint[] memory amounts);
}

/* -------------------------------- BIGGI ERC20 ------------------------------ */
contract BiggiToken is ERC20, ERC20Capped, Ownable {
    // CAP: 10,000,000 BIGGI (18 decimals)
    ...
    /// @notice Mintuje pouze vlastník (Rewards&Liquidity kontrakt), kontrola capu je v ERC20Capped.
    function mint(address to, uint256 amount) external onlyOwner {
        _mint(to, amount);
    }

    /// @notice Kolik ještě lze vymintovat do capu.
...
contract BiggiRewardsAndLiquidity is Ownable, ReentrancyGuard {
    ...
    // === Nové nastavitelné likviditní parametry ===
    ...
    uint256 public lpAddSlippageBps = 0;      // min tolerance pro addLiquidityETH na obě strany (0–10000)
        ...
        Ownable(owner_)
    {
        ...
        // Router může být nastaven až PO deployi (router_ může být 0x0)
    ...
    /// @notice Týdenní claim (počítá váhy dle blockIdx), respektuje cap BIGGI tokenu.
    function claim(uint256[] calldata tokenIds) external nonReentrant {
        ...
        unchecked {
            for (uint256 i = 0; i < tokenIds.length; ++i) {
                ...
                uint16 blk = mainNFT.blockOf(tid); // 1..10 (0 = neplatné/ticket)
    ...
    /// @notice Nastaví příjemce LP tokenů (default owner()).
    ...
    /// @notice Nastaví deadline pro router operace v sekundách.
    ...
    /// @notice Nastaví swap slippage (0–10000 = 0–100 %). Použito pro amountOutMin u swapu.
    ...
    /// @notice Nastaví min. toleranci pro addLiquidityETH (obě strany) v bps (0–10000).
    ...
    /**
     * @notice Nastaví custom swap path (WNATIVE → ... → BIGGI). Délka 2–5.
     * První adresa musí být wrappedNative, poslední adresa BIGGI.
     */
    ...
    /// @notice Zruší custom path a vrátí se na default [wrappedNative, BIGGI].
    ...
    /**
     * @notice PRVNÍ založení LP (bez swapu).
     * - Pošli nativní měnu v msg.value (ETH/MATIC)
     * - Kontrakt si domintuje `tokenAmount` (respektuje cap)
     * - Vytvoří se BIGGI/WETH LP a LP tokeny jdou `liquidityRecipient`
     *
     * Slippage (lpAddSlippageBps) se promítne do amountTokenMin/amountETHMin.
     */
    function bootstrapLiquidity(uint256 tokenAmount) external payable onlyOwner nonReentrant whenRouterSet {
        ...
        // Mint do kontraktu (revertne, pokud by cap nestačil)
        biggi.mint(address(this), tokenAmount);

        IERC20(address(biggi)).approve(address(router), tokenAmount);
    ...
    /**
     * @notice Další doplňování LP z nativního zůstatku:
     * - půlka se vymění přes router za BIGGI (vyžaduje existující pool)
     * - druhá půlka + nabyté BIGGI se přidá do LP
     * amountOutMin pro swap je dopočítán přes getAmountsOut a swapSlippageBps.
     * amountTokenMin/amountETHMin při addLiquidityETH se řídí lpAddSlippageBps (pro jistotu).
     */
    function addLiquidityFromBalance() external onlyOwner nonReentrant whenRouterSet {
        ...
        // === swap: WETH/WMATIC -> (..optional hops..) -> BIGGI
        address[] memory path = _swapPath();

        uint amountOutMin = 0;
        // Pokus o quote (safe – v nejhorším zůstane 0)
        ...
        IERC20(address(biggi)).approve(address(router), tokenBal);
    ...
    /// @notice Adresa BIGGI tokenu pro UI.
    function tokenAddress() external view returns (address) {
        return address(biggi);
    }

    /// @notice Základní metadata tokenu (pro snadné načtení ve FE).
    ...
    /// @notice Kolik BIGGI lze ještě celkem emitovat (CAP - totalSupply).
    function remainingCap() external view returns (uint256) {
        return biggi.remainingMintable();
    }

    /// @notice Váhy bloků (index 0 je „nepoužito“, 1..10 platné).
    function getBlockWeights() external view returns (uint8[11] memory w) {
        w = blockWeight;
    }

    /// @notice Náhled budoucího volání addLiquidityFromBalance() podle aktuálního zůstatku.
    ...
    /// @notice Odhad, kolik by šlo právě teď vyclaimovat za zadané tokeny.
    ...
    /// @notice Detailní stav pro FE: zda je token claimovatelný tento týden + jeho váha a blockIdx.
    ...
    // @dev vrátí aktivní path: custom nebo default [wrappedNative -> BIGGI]
    ...
    function _setRouter(address router_) internal {
        ...
        // pokud existovala custom path, ověř prvky (první musí být nové wrappedNative, poslední BIGGI)
        if (_customSwapPath.length > 0) {
            if (_customSwapPath[0] != wrappedNative || _customSwapPath[_customSwapPath.length - 1] != address(biggi)) {
                delete _customSwapPath; // invalidováno změnou routeru/WNATIVE
                emit SwapPathCleared();
            }
        }
    }
    ...
    function _applyBps(uint amount, uint bps) internal pure returns (uint minOut) {
        if (bps == 0) return 0; // pro addLiquidityETH očekává amountMin, 0 = bez omezení
        // zde vracíme "min" jako amount po snížení o bps (např. 100 * (1 - 0.01) = 99)
