...
contract BiggiRewardsAndLiquidity is Ownable, ReentrancyGuard {
    ...
    /// @dev Treasury si BIGGI stáhne voláním transferFrom – musíte:
    ///      1) v Treasury povolit tento kontrakt jako operátora (setOperator(this, true))
    ///      2) zde schválit treasury jako spender (approve)
    function routeBiggiToTreasury(uint256 amount) external onlyOwner nonReentrant {
        ...
        IERC20(address(biggi)).approve(address(treasury), amount);
        treasury.depositAndSplit(amount); // treasury provede transferFrom(this, treasury, amount)
        ...
        nonReentrant
    {
        ...
        IERC20(address(biggi)).approve(address(treasury), acquired);
