 ...
 * - přijímá MATIC z Distributora přes depositMaticFromDistributor() a drží ho jako emergency
 */
...
contract BiggiTreasury is Ownable {
    ...
    address public distributor;      // adresa Distributor kontraktu (posílá MATIC)
    ...
    event MaticFromDistributor(uint256 amount, uint256 totalReceived);
    ...
    /* ===== Inflow MATIC z Distributoru (10% podíl apod.) =====
           Distributor musí volat tuto funkci. MATIC zůstanou v treasury. */
    function depositMaticFromDistributor() external payable {
        ...
        emit MaticFromDistributor(msg.value, totalMaticReceived);
    }
    ...
    */
    ...
    function totalMaticReceivedFromDistributor() external view returns (uint256) {
        return totalMaticReceived;
    }
