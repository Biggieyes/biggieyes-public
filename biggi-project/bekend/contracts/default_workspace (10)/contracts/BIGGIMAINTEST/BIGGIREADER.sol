...
/* ---------- MINIMAL VIEW INTERFACE NA NOVÝ MAIN (rozšířeno o background/block checky) ---------- */
interface IBiggiMainView {
    ...
    // ---- NOVÉ: helpery potřebné pro CollectionRewards ----
    // vrátí true pokud owner vlastní všechny background varianty (včetně O) pro tento mainId v daném bloku
    function hasAllBackgroundsForMainIdInBlock(address owner, uint16 blk, uint256 mainId) external view returns (bool);
    // vrátí true pokud owner vlastní všech 10 mainId v bloku (originální O varianty nebo jak main implementuje)
...
contract BiggiReader {
    ...
    /* ======== NOVÉ WRAPPERY pro FE (pomáhají CollectionRewards UI) ======== */

    function hasAllBackgroundsForMainIdInBlock(address owner, uint16 blk, uint256 mainId) external view returns (bool) {
        return main.hasAllBackgroundsForMainIdInBlock(owner, blk, mainId);
    }

    function hasAllTenMainIdsInBlock(address owner, uint16 blk) external view returns (bool) {
        return main.hasAllTenMainIdsInBlock(owner, blk);
    }
}
