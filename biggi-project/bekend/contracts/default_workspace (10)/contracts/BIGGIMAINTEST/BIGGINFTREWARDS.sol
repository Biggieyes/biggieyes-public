...
interface IBiggiMain {
    // only used for access control (main contract will call notifyLastMint)
    // no other calls required for this contract
}
...
contract BiggiNFTRewards is ERC721, Ownable, ReentrancyGuard {
    ...
    modifier onlyMainContract() {
        require(msg.sender == mainContract, "only main");
        _;
    }
    ...
    */
    ...
    function createCharacterReward(address winner, string calldata tokenUri) external onlyMainContract returns (uint256 eventId, uint256 rewardId) {
        require(winner != address(0), "winner0");
        eventId = nextEventId++;
        rewardId = nextRewardId++;
