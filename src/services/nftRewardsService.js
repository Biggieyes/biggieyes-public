...
const ABI = [
  ...
  { "inputs":[{"internalType":"uint256","name":"rewardId","type":"uint256"}],"name":"claim","outputs":[],"stateMutability":"nonpayable","type":"function"},
  { "inputs":[{"internalType":"uint256","name":"","type":"uint256"}],"name":"assignedTo","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},
  ...
  { "inputs":[{"internalType":"uint256","name":"rewardId","type":"uint256"}],"name":"rewardInfo","outputs":[{"internalType":"address","name":"assigned","type":"address"},{"internalType":"bool","name":"isClaimed","type":"bool"},{"internalType":"string","name":"uri","type":"string"}],"stateMutability":"view","type":"function"},
  { "inputs":[{"internalType":"uint256","name":"","type":"uint256"}],"name":"rewardTokenUri","outputs":[{"internalType":"string","name":"","type":"string"}],"stateMutability":"view","type":"function"},
  { "inputs":[{"internalType":"uint256","name":"","type":"uint256"}],"name":"events","outputs":[{"internalType":"uint8","name":"kind","type":"uint8"},{"internalType":"address","name":"creator","type":"address"},{"internalType":"uint256","name":"rewardStartId","type":"uint256"},{"internalType":"uint256","name":"rewardCount","type":"uint256"},{"internalType":"bool","name":"randomnessRequested","type":"bool"},{"internalType":"bool","name":"finished","type":"bool"},{"internalType":"uint256","name":"vrfRequestId","type":"uint256"}],"stateMutability":"view","type":"function"},
  { "inputs":[{"internalType":"uint256","name":"eventId","type":"uint256"}],"name":"eventEligibleCount","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},
  { "inputs":[{"internalType":"uint256","name":"eventId","type":"uint256"},{"internalType":"uint256","name":"idx","type":"uint256"}],"name":"getEligibleAt","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},
  { "inputs":[],"name":"nextEventId","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},
  { "inputs":[],"name":"nextRewardId","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},
  { "inputs":[{"internalType":"uint256","name":"","type":"uint256"}],"name":"vrfRequestToEvent","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"},
  { "inputs":[],"name":"vrfRouter","outputs":[{"internalType":"address","name":"","type":"address"}],"stateMutability":"view","type":"function"},
...
export default class NFTRewardsService {
  ...
  // ----------- Read getters -----------
  async balanceOf(ownerAddr) { return await this.contract.balanceOf(ownerAddr); }
  async ownerOf(tokenId) { return await this.contract.ownerOf(tokenId); }
  async name() { return await this.contract.name(); }
  async symbol() { return await this.contract.symbol(); }
  async tokenURI(tokenId) { return await this.contract.tokenURI(tokenId); }
  async supportsInterface(interfaceId) { return await this.contract.supportsInterface(interfaceId); }

  // rewards state
  async assignedTo(rewardId) { return await this.contract.assignedTo(rewardId); }
  async claimed(rewardId) { return await this.contract.claimed(rewardId); }
  async rewardInfo(rewardId) { return await this.contract.rewardInfo(rewardId); } // {assigned, isClaimed, uri}
  async rewardTokenUri(rewardId) { return await this.contract.rewardTokenUri(rewardId); }

  // events / mystery
  async events(idx) { return await this.contract.events(idx); } // struct
  async eventEligibleCount(eventId) { return await this.contract.eventEligibleCount(eventId); }
  async getEligibleAt(eventId, idx) { return await this.contract.getEligibleAt(eventId, idx); }
  async nextEventId() { return await this.contract.nextEventId(); }
  async nextRewardId() { return await this.contract.nextRewardId(); }

  // vrf / helpers
  async vrfRequestToEvent(reqId) { return await this.contract.vrfRequestToEvent(reqId); }
  async vrfRouter() { return await this.contract.vrfRouter(); }
  async mainContract() { return await this.contract.mainContract(); }
  async owner() { return await this.contract.owner(); }
  async isApprovedForAll(ownerAddr, operatorAddr) { return await this.contract.isApprovedForAll(ownerAddr, operatorAddr); }
  async getApproved(tokenId) { return await this.contract.getApproved(tokenId); }
  ...
  async getAllStats() {
    ...
    const [name, symbol, nextEventId, nextRewardId, vrfRouter, mainContract, owner] = await Promise.all(calls);
    return { name, symbol, nextEventId, nextRewardId, vrfRouter, mainContract, owner };
  }
  ...
  async fetchEventsDetailed({ includeEligible = false, limit = 100 } = {}) {
    ...
    const promises = ids.map(async (id) => {
      ...
      const eligibleCount = Number(eligibleCountBn?.toString?.() ?? eligibleCountBn);
      let eligible = [];
      if (includeEligible && eligibleCount > 0) {
        // opatrně - volá RPC eligibleCount krát
        const ePromises = [];
        for (let j = 0; j < eligibleCount; j++) ePromises.push(this.getEligibleAt(id, j));
  ...
  async fetchUserAssignedRewards(userAddr, { limit = 1000 } = {}) {
    ...
    for (let i = 0; i < max; i++) {
      ...
      if (String(info.assigned).toLowerCase() === String(userAddr).toLowerCase()) {
