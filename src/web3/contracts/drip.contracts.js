...
export function getDripContracts(chainId, provider) {
  ...
  const { dripDistributor, dripLM, biggiToken, router, reserve, treasury } = getDripAddresses(chainId);

  const distributorContract = new Contract(dripDistributor, DripDistributor, signerOrProvider);
