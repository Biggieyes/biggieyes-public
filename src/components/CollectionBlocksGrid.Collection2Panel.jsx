...
const Collection2Panel = React.memo(({ 
  ...
  const ticketsPct = React.useMemo(() => {
    ...
    return (collectionTotals.ticketMinted / collectionTotals.maxTickets) * 100;
  }, [collectionTotals?.ticketMinted, collectionTotals?.maxTickets]);

  const nativeRevenue = collectionTotals?.totalRaisedNative ?? collectionTotals?.nativeRevenue ?? null;
  const nativeRevenueLabel = formatPrice(nativeRevenue);

  const formatPct = (val) => (Number.isFinite(val) ? `${val.toFixed(1)}%` : FALLBACK_VALUE);
  const mintedBarWidth = mintedPct ? Math.min(mintedPct, 100).toFixed(1) : '12';
  const revenueSpark = React.useMemo(() => {
    ...
    const coords = normalized.map((v, i) => `${i * (100 / (normalized.length - 1))},${100 - (v / max) * 100}`);
    return coords.join(' ');
    ...
    <section className="collection-grid__panel collection-grid__panel--glass">
      ...
      <header className="collection-grid__panel-header collection-grid__panel-header--stacked">
        <div className="collection-grid__panel-title">
          <div className="collection-grid__pill collection-grid__pill--gradient">Public mint</div>
          <h3>Collection 2</h3>
        </div>
        <p className="collection-grid__panel-subtitle">Live pricing, availability, and a streamlined mint helper for every block.</p>
      </header>
      ...
      <div className="collection-grid__panel collection-grid__panel--glass-inner">
        <div className="collection-grid__two-column collection-grid__two-column--balanced">
          <div className="collection-grid__cardbox collection-grid__cardbox--frosted">
            <div className="collection-grid__cardbox-head">
              <h4>Mint setup</h4>
              <span className="collection-grid__pill collection-grid__pill--outline">collectionPublic</span>
            </div>

            <form className="collection-grid__form" onSubmit={(e) => e.preventDefault()}>
              <label className="collection-grid__form-field">
                <span>Block</span>
                <select
                  name="block"
                  value={selectedBlock}
                  onChange={(event) => onBlockChange(Number(event.target.value) || 1)}
                >
                  {blockEntries.filter((e) => e.hasData).map((e, i) => (
                    <option key={e.id} value={i + 1}>{e.name || `Block ${i + 1}`}</option>
                  ))}
                </select>
              </label>
              ...
              <label className="collection-grid__form-field">
                ...
                <select
                  ...
                  onChange={(event) => onBackgroundChange(Number(event.target.value) || 1)}
                >
                  {Array.from({ length: 10 }, (_, i) => i + 1).map((bg) => (
                    <option key={bg} value={bg}>Background {bg}</option>
                  ))}
                </select>
              </label>
          ...
          <div className="collection-grid__cardbox collection-grid__cardbox--frosted">
            <div className="collection-grid__cardbox-head">
              <h4>Mint details</h4>
              <span className="collection-grid__pill collection-grid__pill--soft">On-chain</span>
            </div>
            <dl className="collection-grid__key-values">
              <div><dt>Block price</dt><dd>{formatPrice(selectedEntry?.currentPrice)}</dd></div>
              <div><dt>Minted in block</dt><dd>{formatCount(selectedEntry?.minted)}</dd></div>
              <div><dt>Selected NFT</dt><dd>{desiredTokenId ? `#${desiredTokenId}` : "Not set"}</dd></div>
            </dl>
            ...
            <button type="button" className="collection-grid__action-btn collection-grid__action-btn--primary" disabled>
              Mint NFT (publicMintNFTById)
            </button>
            <p className="collection-grid__helper">Live data, action disabled in preview.</p>
          </div>
        </div>
        ...
        <div className="collection-grid__stat-cards collection-grid__stat-cards--wide">
          <div className="collection-grid__stat-card collection-grid__stat-card--glass">
            ...
            <span className="collection-grid__stat-value-large">{formatCount(collectionTotals.biggiMinted)}</span>
            <span className="collection-grid__stat-sub">{collectionTotals.maxSupply ? `z ${collectionTotals.maxSupply}` : FALLBACK_VALUE}</span>
            <div className="collection-grid__progress">
              <div className="collection-grid__progress-bar" style={{ width: mintedPct ? `${Math.min(mintedPct, 100).toFixed(1)}%` : '0%' }} />
            </div>
            <span className="collection-grid__stat-foot">{formatPct(mintedPct)}</span>
          </div>
          <div className="collection-grid__stat-card collection-grid__stat-card--glass">
            <span className="muted">Revenue (native)</span>
            <span className="collection-grid__stat-value-large">{nativeRevenueLabel || FALLBACK_VALUE}</span>
            <span className="collection-grid__stat-sub">Total earned on-chain</span>
            {revenueSpark ? (
              <div className="collection-grid__sparkline" aria-label="Revenue trend">
                <svg viewBox="0 0 100 100" preserveAspectRatio="none">
                  <polyline points={revenueSpark} />
                </svg>
              </div>
            ) : (
              <div className="collection-grid__sparkline collection-grid__sparkline--empty">Waiting for trend</div>
            )}
            <span className="collection-grid__stat-foot">Live snapshot</span>
          </div>
          <div className="collection-grid__stat-card collection-grid__stat-card--glass">
            <span className="muted">Status</span>
            <span className="collection-grid__stat-value-large">{collectionTotals.paused ? "Paused" : "Live"}</span>
      ...
      <section className="collection-grid__cards-panel">
        <div className="collection-grid__cards-heading">
          <div>
            <p className="collection-grid__chip collection-grid__chip--ghost">Mintable blocks</p>
            <h4 className="collection-grid__cards-title">Pick a block and background, then mint.</h4>
          </div>
          <p className="collection-grid__cards-sub">Five tiles per row on desktop; smaller screens auto-collapse.</p>
        </div>
