...
const ensureRanks = (ranks, factory) => {
  const base = typeof factory === "function" ? factory() : { 1: false, 2: false, 3: false };
  ...
  return (
    <div className="rewards-grid__preview-grid">
      {Array.from({ length: safeCount }).map((_, index) => {
        ...
        return (
          <div key={`${prefix}-${index}`} className="rewards-grid__preview-thumb">
            <img
              src={src}
            ...
            />
            <span className="rewards-grid__preview-caption">NFT #{index + 1}</span>
          </div>
        );
      })}
    </div>
...
function NftRewardsSection({
  ...
  const normalizedRange = Array.isArray(range) && range.length ? range : FALLBACK_RANGE;
  const mintedLabel = typeof formatInteger === "function" ? formatInteger(totalMinted) : String(totalMinted ?? 0);
  const formattedAddress = typeof formatAddress === "function" ? formatAddress(contractAddress) : contractAddress || FALLBACK_ADDRESS;
  const characterUri = typeof formatUriDisplay === "function" ? formatUriDisplay(baseURIs.character) : baseURIs.character || FALLBACK_URI_LABEL;
  const leaderboardUri = typeof formatUriDisplay === "function" ? formatUriDisplay(baseURIs.leaderboard) : baseURIs.leaderboard || FALLBACK_URI_LABEL;
  const mysteryUri = typeof formatUriDisplay === "function" ? formatUriDisplay(baseURIs.mystery) : baseURIs.mystery || FALLBACK_URI_LABEL;
  ...
  const mintedLeaderboard = Object.values(leaderboardClaimed || {}).reduce((acc, ranks) => acc + countTruthy(ranks), 0);
  ...
  const tableRows = normalizedRange.map((block) => {
    ...
    return {
      ...
      leaderboardTone: lbCount === 3 ? "is-claimed" : lbCount > 0 ? "is-open" : "",
    ...
    <section className="rewards-panel__section rewards-panel__section--nft nft-rewards">
      ...
      <div className="rewards-panel__grid">
        <div className="nft-rewards__summary-card biggi-card biggi-card--y rewards-panel__card">
          ...
          <div className="biggi-card__body">
            ...
            <span className="nft-rewards__summary-hint">Character + Leaderboard + Mystery drops</span>
          </div>
        </div>
        <div className="nft-rewards__summary-card biggi-card biggi-card--c rewards-panel__card">
          <div className="biggi-card__glow" aria-hidden />
          <div className="biggi-card__body">
            <span className="nft-rewards__summary-label">Rewards contract</span>
            <span className="nft-rewards__summary-value">{formattedAddress}</span>
            <span className="nft-rewards__summary-hint">Polygon Amoy deployment</span>
            <button
              type="button"
              className="nft-rewards__summary-btn biggi-btn biggi-btn--ghost"
              onClick={() => !explorerDisabled && typeof onOpenExplorer === "function" && onOpenExplorer(contractAddress)}
      ...
      <div className="rewards-panel__grid nft-rewards__cards">
        <article className="biggi-card biggi-card--v rewards-panel__card nft-rewards__card">
          ...
          <div className="biggi-card__header">
            <div className="biggi-card__heading">
              ...
              <p>One Character reward per block after requirements are met.</p>
            </div>
          ...
          <div className="biggi-card__body">
            {renderStaticPreview("/images/rewards/characters", characterCount, "Character")}

            <div className="nft-rewards__status-list">
              {normalizedRange.map((block) => {
                const claimed = Boolean(characterClaimed?.[block]);
                return (
                  <div key={`char-${block}`} className="nft-rewards__status-row">
                    <div className="nft-rewards__status-meta">
                      <span>Block {block}</span>
                      <small>1 Character NFT</small>
                    </div>
                    <span className={`nft-rewards__status-pill ${claimed ? "is-claimed" : "is-open"}`}>
            ...
            <ul className="rewards-grid__info-list">
              ...
              <li>Players unlock Character drops by completing all ten backgrounds.</li>
            </ul>
          </div>
        </article>
        ...
        <article className="biggi-card biggi-card--c rewards-panel__card nft-rewards__card">
          ...
          <div className="biggi-card__body">
            <div className="nft-rewards__leaderboard-grid">
              {normalizedRange.map((block) => {
                const ranks = ensureRanks(leaderboardClaimed?.[block], emptyRanks);
                return (
                  <div key={`lb-${block}`} className="nft-rewards__leaderboard-row">
                    <span className="nft-rewards__status-label">Block {block}</span>
                    <div className="nft-rewards__badge-set">
                      {[1, 2, 3].map((rank) => (
                        <span key={`${block}-${rank}`} className={`nft-rewards__badge ${ranks?.[rank] ? "is-claimed" : ""}`}>
                          #{rank}
                        </span>
                      ))}
                    </div>
        ...
        <article className="biggi-card biggi-card--y rewards-panel__card nft-rewards__card">
          ...
          <div className="biggi-card__body">
            {renderStaticPreview("/images/rewards/rainbowNFT", mysteryCount, "Mystery")}

            <div className="nft-rewards__status-list">
              {normalizedRange.map((block) => {
                const claimed = Boolean(mysteryClaimed?.[block]);
                return (
                  <div key={`mys-${block}`} className="nft-rewards__status-row">
                    <div className="nft-rewards__status-meta">
                      <span>Block {block}</span>
            ...
            <ul className="rewards-grid__info-list">
              <li>One Mystery NFT per block, revealed after metadata is published.</li>
              <li>Metadata is served from the Mystery base URI listed above.</li>
            </ul>
            ...
            {contractAddress && (
              <div className="rewards-grid__actions">
                <button
                  ...
                  onClick={() => typeof onOpenExplorer === "function" && onOpenExplorer(contractAddress)}
                >
        ...
        <article className="biggi-card biggi-card--v rewards-panel__card nft-rewards__card nft-rewards__table-card">
          ...
          <div className="biggi-card__header">
            <div className="biggi-card__heading">
              ...
              <p>Combined view of Character, Leaderboard, and Mystery mints.</p>
            </div>
          </div>
          <div className="biggi-card__body">
              ...
              <tbody>
                {tableRows.map((row) => (
                  <tr key={`tbl-${row.block}`}>
                    ...
                    <td>
                      <span className={`nft-rewards__pill ${row.characterTone}`}>{row.character}</span>
                    </td>
                    <td>
                      <span className={`nft-rewards__pill ${row.leaderboardTone}`}>{row.leaderboard}</span>
                    </td>
                    <td>
                      <span className={`nft-rewards__pill ${row.mysteryTone}`}>{row.mystery}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
