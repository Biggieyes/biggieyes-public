 ...
 */
export default function InfoPanel({ children, compact = false, data = null, loading = false, onRefresh }) {
  ...
  return (
    <section
      ...
      tabIndex={-1}
      style={{ padding: compact ? 12 : 20, display: "grid", gap: compact ? 12 : 16 }}
    >
      {hasData && <DynamicContent data={data} compact={compact} loading={loading} onRefresh={onRefresh} />}
...
function ExplorerLink({ addr, label }) {
  ...
  return (
    <a href={url} target="_blank" rel="noreferrer" style={{ color: "var(--biggi-accent, #6cf)" }}>
      {label}
    </a>
  );
}
  ...
  return (
    <div style={{ display: "grid", gap: compact ? 12 : 16 }}>
      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <Badge>{rpc?.latencyMs != null ? `latence ${rpc.latencyMs} ms` : "latence n/a"}</Badge>
        <Badge>{rpc?.error ? `RPC error: ${rpc.error}` : "RPC OK"}</Badge>
        {onRefresh && (
          <button
            type="button"
            onClick={onRefresh}
            style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.15)", background: "transparent", color: "inherit", cursor: "pointer" }}
          >
      ...
      <Block title="Snapshot (reader)" compact={compact}>
        ...
        <Stat label="Reader" value={<ExplorerLink addr={addresses?.reader} label={shortAddr(addresses?.reader)} />} />
      </Block>
      ...
      <Block title="Policy / tokenomika" compact={compact}>
        ...
        <Stat label="Main" value={<ExplorerLink addr={addresses?.main} label={shortAddr(addresses?.main)} />} />
        <Stat label="Rewards" value={<ExplorerLink addr={addresses?.rewards} label={shortAddr(addresses?.rewards)} />} />
        <Stat label="Buyback" value={<ExplorerLink addr={addresses?.buyback} label={shortAddr(addresses?.buyback)} />} />
      </Block>
    </div>
  );
}
...
function Block({ title, children, compact }) {
  return (
    <div style={{ display: "grid", gap: compact ? 6 : 8 }}>
      <h3 style={{ margin: 0, fontSize: compact ? 16 : 18, lineHeight: 1.25 }}>{title}</h3>
      <div style={{ fontSize: compact ? 13 : 14, lineHeight: 1.6, opacity: 0.92 }}>{children}</div>
    </div>
  );
}
...
function Stat({ label, value, accent }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
      ...
      <strong style={{ color: accent ?? "var(--biggi-accent, #6cf)", fontVariantNumeric: "tabular-nums" }}>{value}</strong>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "grid", gap: compact ? 14 : 18 }}>
      ...
      <Block title="Co stavíme" compact={compact}>
        Gamifikovanou on-chain NFT loterii s dynamickým pricingem bloků, integrovaným tokenem BIGGI a týdenními reward pooly.
        Mintování i výplaty jsou on-chain; front-end používá read-only providery na Polygon Amoy.
      </Block>

      <Block title="Jak to funguje" compact={compact}>
        <ul style={{ margin: 0, paddingLeft: 18, display: "grid", gap: 6 }}>
          <li>Bloky NFT mají rostoucí ceny; ceny i počty mintů čte reader kontrakt.</li>
          <li>VRF router zajišťuje férové losování; odměny se vyplácí z reward poolu.</li>
          <li>Buyback a treasury kontrakty spravují tokonomiku a udržitelnost poolů.</li>
          <li>Liquidity manager drží POL likviditu a krmí pooly odměn podle parametrů politiky.</li>
        </ul>
      </Block>

      <Block title="Tokenomika (high-level)" compact={compact}>
        <ul style={{ margin: 0, paddingLeft: 18, display: "grid", gap: 6 }}>
          <li>Primární tok: mint -&gt; fee -&gt; rozdělení mezi treasury, rewards, buyback.</li>
          <li>Weekly reward pool: dynamický podíl z mint volume (viz kontrakt LM/Rewards).</li>
          <li>Buyback agent: může rekapitalizovat odměny nebo řídit spalování tokenu.</li>
        </ul>
      </Block>
      ...
      <Block title="Roadmap / stav" compact={compact}>
        <ul style={{ margin: 0, paddingLeft: 18, display: "grid", gap: 6 }}>
          ...
          <li>🛠 Monitoring RPC spolehlivosti: preferujeme publicnode, bez Ankr fallbacku.</li>
          <li>🔜 Přidat další veřejné RPC jako záložní (pokud bude potřeba).</li>
        </ul>
      </Block>

      <Block title="Rychlé tipy" compact={compact}>
        <ul style={{ margin: 0, paddingLeft: 18, display: "grid", gap: 6 }}>
          <li>Pokud se nenačtou data, zkontroluj síť (Polygon Amoy) a obnov RPC (localStorage: biggi_last_amoy_rpc_v1).</li>
          <li>Pro jistotu nech povolený fallback jen na public node (viz .env: VITE_JSON_RPC_URL).</li>
          <li>VRF a odměny jsou read-only — chyby typu CALL_EXCEPTION znamenají nejčastěji špatný RPC nebo timeout.</li>
          <li>UI čte jen read-only; transakce na testnetu nevyžadují soukromé klíče v prohlížeči mimo podpisů.</li>
        </ul>
      </Block>
