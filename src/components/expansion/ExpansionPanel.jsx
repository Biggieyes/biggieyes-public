...
const THEME = {
  ...
  accentSoft: "linear-gradient(135deg, rgba(255,232,0,0.06), rgba(93,220,255,0.03))"
};
...
const styles = {
  ...
  headerCard: {
    ...
    boxShadow: "0 12px 40px rgba(0,0,0,0.6), 0 0 30px rgba(255,232,0,0.03) inset",
    display: "flex",
  ...
  rightPanel: {
    borderRadius: 14,
    padding: 16,
    background: "linear-gradient(180deg, rgba(255,255,255,0.01), rgba(0,0,0,0.13))",
    border: `1px solid ${THEME.border}`,
  ...
  modalCard: {
    ...
    boxShadow: "0 40px 100px rgba(0,0,0,0.8), 0 0 60px rgba(255,232,0,0.05) inset",
    maxHeight: "90vh",
    overflowY: "auto",
  },
  placeholderGrid: {
  ...
  placeThumb: {
    ...
    height: 120,
    borderRadius: 12,
    background: "linear-gradient(135deg, rgba(255,232,0,0.1), rgba(93,220,255,0.05))",
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    color: THEME.gold,
...
export default function ExpansionPanel() {
  ...
  React.useEffect(() => {
    const init = async () => {
      try {
        ...
        const prov = typeof window !== "undefined" && window.ethereum
          ? new ethers.providers.Web3Provider(window.ethereum, "any")
          : getROProvider();
        setProvider(prov);
        // nastav contract proti read-only provider (ne defaultProvider)
        setContract(new ethers.Contract(DISTRIBUTOR_ADDRESS, DISTRIBUTOR_ABI, prov));
      } catch (e) {
        console.error(e);
  ...
  const connect = React.useCallback(async () => {
    try {
      if (!window.ethereum) { setStatus("No injected wallet"); return; }
  ...
  const loadOnChain = React.useCallback(async () => {
    ...
    try {
      ...
      const evts = await contract.queryFilter(contract.filters.MintShareAccepted(), fromBlock, latest || "latest");
      const mapped = (evts || []).slice(-12).reverse().map(e => {
        const col = e.args && e.args.collection ? e.args.collection : (e.args[0] || null);
        const amt = e.args && e.args.amount ? e.args.amount : (e.args[1] || ethers.BigNumber.from(0));
        return {
  ...
  React.useEffect(() => { loadOnChain(); }, [contract, account, loadOnChain]);

  React.useEffect(() => {
    if (!contract || !contract.on) return;
    const handler = (collection, amount, ev) => {
      const item = { tx: ev.transactionHash, block: ev.blockNumber, collection: String(collection), amount: ethers.utils.formatEther(amount) };
      setEvents(prev => [item, ...prev].slice(0, 12));
      loadOnChain();
    };
    contract.on(contract.filters.MintShareAccepted(), handler);
    return () => { try { contract.off(contract.filters.MintShareAccepted(), handler); } catch (e) {} };
  ...
  const openExplorer = React.useCallback(async () => {
    const net = provider ? await provider.getNetwork().then(n => n.name).catch(() => "") : "";
        ...
        <div style={styles.headerCard}>
          <div style={styles.titleGroup}>
            ...
            <div>
              ...
              <p style={styles.subtitle}>User dashboard — overview of the mint-share flow and your collection status</p>
            </div>
          </div>

          <div style={styles.headerActions}>
            <button onClick={() => setFutureOpen(true)} style={styles.primaryBtn}>
              FutureCollection
            </button>
            <button onClick={openExplorer} style={styles.explorerBtn}>Open explorer</button>
          </div>
        </div>
        ...
        <div style={{ gridColumn: "1 / -1", display: "grid", gap: 12 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "stretch", flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 220 }}>
              <div style={styles.statCard}>
                ...
                <div style={{ color: THEME.dim, fontSize: 12, marginTop: 6 }}>Total POL received by the Distributor contract</div>
              </div>
            </div>
            ...
            <div style={{ flex: 1, minWidth: 220 }}>
              <div style={styles.statCard}>
                ...
                <div style={{ fontSize: 20, fontWeight: 900, color: THEME.cyan }}>{receivedForAddr} POL</div>
                <div style={{ color: THEME.dim, fontSize: 12, marginTop: 6 }}>{isWhitelisted ? "You are whitelisted" : "Not whitelisted"}</div>
              </div>
            </div>
            ...
            <div style={{ flex: 1, minWidth: 220 }}>
              <div style={styles.statCard}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <button onClick={copyAddress} style={{ ...styles.ghostBtn, flex: 1 }} disabled={!account}>Copy my address</button>
                  <button onClick={() => navigator.clipboard?.writeText(DISTRIBUTOR_ADDRESS)} style={{ ...styles.ghostBtn, flex: 1 }}>Copy distributor</button>
                </div>
                <div style={{ color: THEME.dim, fontSize: 12, marginTop: 8 }}>Read-only UI for users — for whitelist requests contact the project team.</div>
              </div>
            </div>
          </div>
          ...
          <div style={styles.leftPanel}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 800, color: THEME.gold }}>What is Distributor?</div>
                <div style={styles.infoText}>
                  The Distributor receives the mint-share (part of every mint) from whitelisted collections and routes it into reserves, rewards, buyback, and the treasury.
                  In this user panel you can see how much was received in total, how much was credited to your collection, and the most recent mint-share events.
                </div>
              </div>
              <div style={{ minWidth: 160 }}>
                <div style={{ fontSize: 13, color: THEME.dim, marginBottom: 8 }}>Status</div>
                <div style={{ padding: 10, borderRadius: 10, background: THEME.glass, border: "1px solid rgba(255,255,255,0.02)" }}>
                  <div style={{ fontWeight: 800, color: THEME.cyan }}>{isWhitelisted ? "Whitelisted" : "Not whitelisted"}</div>
                  <div style={{ color: THEME.dim, marginTop: 6 }}>{status || "Ready"}</div>
                </div>
              </div>
            </div>
            ...
            <div style={styles.eventsCard}>
              <div style={styles.eventsHeader}>
                <div style={{ fontWeight: 800, color: THEME.gold }}>Recent mint-share events</div>
                <div style={{ color: THEME.dim, fontSize: 13 }}>{events.length} items</div>
              </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead>
                      <tr style={{ color: THEME.dim, textAlign: "left" }}>
                        ...
                        <th style={{ padding: "8px 6px", textAlign: "right" }}>Block</th>
                      </tr>
                    </thead>
                    <tbody>
                      {events.map((ev, i) => (
                        <tr key={i} style={{ borderTop: "1px solid rgba(255,255,255,0.02)" }}>
                          <td style={{ padding: "10px 6px", color: "#e9f9ff" }}>{shortAddr(ev.collection)}</td>
                          <td style={{ padding: "10px 6px", textAlign: "right", color: "#fff" }}>{ev.amount}</td>
                          <td style={{ padding: "10px 6px", textAlign: "right", color: THEME.dim }}>{ev.block}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <aside style={styles.rightPanel}>
            <div>
              <div style={{ fontWeight: 800, color: THEME.gold, marginBottom: 6 }}>Your quick tools</div>
              <div style={{ display: "grid", gap: 8 }}>
                <button onClick={connect} style={styles.primaryBtn}>{account ? "Connected" : "Connect Wallet"}</button>
                <button onClick={loadOnChain} style={styles.ghostBtn}>Refresh on-chain</button>
                <button onClick={openExplorer} style={styles.ghostBtn}>Open distributor on explorer</button>
                <div style={{ marginTop: 8, color: THEME.dim }}>
                  <div style={{ fontSize: 12, marginBottom: 6 }}>Tip</div>
                  <div style={{ fontSize: 13 }}>If you manage a collection and want to be whitelisted, contact the project team. This panel is informational only.</div>
                </div>
              </div>
            </div>

            <div>
              <div style={{ fontWeight: 800, color: THEME.cyan, marginBottom: 6 }}>Mini status</div>
              <div style={{ background: "rgba(255,255,255,0.02)", padding: 10, borderRadius: 10 }}>
                <div style={{ color: THEME.dim }}>Contract</div>
                <div style={{ color: THEME.gold, fontWeight: 800, marginTop: 6 }}>{DISTRIBUTOR_ADDRESS ? shortAddr(DISTRIBUTOR_ADDRESS) : "-"}</div>
            ...
            <div style={{ color: THEME.dim, fontSize: 12, marginTop: 6 }}>
              <strong>Security</strong>: never call admin functions when the UI is unfamiliar. This panel does not change anything on-chain (read-only) — apart from an optional wallet connection.
            </div>
          </aside>
        </div>
      </div>
        ...
        <div style={styles.modalOverlay} onClick={() => setFutureOpen(false)}>
          <div style={styles.modalCard} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 20, marginBottom: 24 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 900, color: THEME.gold, fontSize: 24, marginBottom: 8 }}>Future Collections</div>
                <div style={{ color: THEME.dim, fontSize: 15, lineHeight: 1.5 }}>
                  Discover upcoming NFT collections in our ecosystem. Each collection brings unique artwork and utility to the platform.
                </div>
              </div>
              <div>
                <button onClick={() => setFutureOpen(false)} style={{ ...styles.ghostBtn, padding: "10px 16px", border: `1px solid ${THEME.border}` }}>Close</button>
              </div>
            </div>
              ...
              {futureCollections.map((collection) => (
                <div key={collection.id} style={styles.placeCard}>
                  ...
                  <div style={styles.placeThumb}>
                    {collection.name.split(' ').map(word => word[0]).join('')}
                  </div>
                  
                  <div style={{ width: "100%", textAlign: "center" }}>
                    <div style={{ color: THEME.gold, fontWeight: 800, fontSize: 18, marginBottom: 6 }}>{collection.name}</div>
                    <div style={{ color: THEME.dim, fontSize: 13, lineHeight: 1.4, marginBottom: 12 }}>{collection.description}</div>
                  </div>

                  <div style={styles.progressBar}>
                    <div style={{ ...styles.progressFill, width: `${collection.progress}%` }} />
                  </div>
                  <div style={{ color: THEME.cyan, fontSize: 12, fontWeight: 700, width: "100%", textAlign: "center" }}>
                    Launch Progress: {collection.progress}%
                  </div>

                  <div style={styles.statsRow}>
                    <div style={styles.statMini}>
                      <div style={{ color: THEME.dim, fontSize: 11 }}>Items</div>
                      <div style={{ color: "#fff", fontWeight: 700, marginTop: 4 }}>{collection.items}</div>
                    </div>
                    <div style={styles.statMini}>
                      <div style={{ color: THEME.dim, fontSize: 11 }}>Mint Price</div>
                      <div style={{ color: THEME.gold, fontWeight: 700, marginTop: 4 }}>{collection.mintPrice}</div>
                    </div>
                  </div>
            ...
            <div style={{ 
              ...
              <div style={{ color: THEME.gold, fontWeight: 700, marginBottom: 8 }}>About Future Collections</div>
              <div style={{ color: THEME.dim, fontSize: 13, lineHeight: 1.6 }}>
                These collections are currently in development and will be integrated with the Distributor system upon launch. 
                Each collection undergoes a thorough review process to ensure quality and compatibility with our ecosystem.
              </div>
            </div>
          </div>
        </div>
