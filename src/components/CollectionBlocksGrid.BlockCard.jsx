...
const BlockCard = React.memo(({
  ...
  return (
    <div
      ...
      onMouseEnter={() => { if (!isTouch) onMouseEnter(entry.name); }}
      onMouseLeave={() => { if (!isTouch) onMouseLeave(); }}
    ...
    >
      ...
      <dl className="collection-grid__meta">
        <div><dt>Live price</dt><dd>{formatPrice(entry.currentPrice)}</dd></div>
        <div><dt>Base price</dt><dd>{entry.basePrice != null ? `${Math.round(entry.basePrice)} POL` : "--"}</dd></div>
        <div><dt>Minted</dt><dd>{formatCount(entry.minted)}</dd></div>
      </dl>

      {entry.diff && (
        <div className={`collection-grid__diff${entry.diff.positive ? " is-positive" : " is-negative"}`}>
          {entry.diff.value}<span>{entry.diff.percent}</span>
        </div>
      )}
