...
export function useAdminActions({
  ...
  return React.useMemo(() => ({
    ...
    liq_setLiquidityRecipient: async (addr) => {
      ...
      await writeFirst([getLiquidityContract], ["setLiquidityRecipient", "setRecipient", "setLiquidityReceiver", "setReceiver"], addr);
    },
    liq_setLpUseBalanceBps: async (bps) => {
      await writeFirst([getLiquidityContract], ["setLpUseBalanceBps", "setLpUseBps", "setUseLpBps"], Number(bps));
    },
    liq_setSwapSlippageBps: async (bps) => {
      await writeFirst([getLiquidityContract], ["setSwapSlippageBps", "setSwapSlipBps", "setSwapSlippage"], Number(bps));
    },
    liq_setLpAddSlippageBps: async (bps) => {
      await writeFirst([getLiquidityContract], ["setLpAddSlippageBps", "setLpSlipBps", "setLpAddSlipBps"], Number(bps));
    },
    liq_setTxDeadline: async (sec) => {
      await writeFirst([getLiquidityContract], ["setTxDeadline", "setTxDeadlineSec", "setDeadline"], Number(sec));
    },
    liq_setSwapPath: async (pathArr) => {
      await writeFirst([getLiquidityContract], ["setSwapPath", "updateSwapPath"], pathArr);
    },
    liq_clearSwapPath: async () => {
      await writeFirst([getLiquidityContract], ["clearSwapPath", "resetSwapPath"], []);
    },
    liq_buybackToTreasury: async (nativeAmount, minOut) => {
      ...
      await writeFirst(
        [getLiquidityContract],
        ["buybackToTreasury", "buyBiggiAndSendToTreasury", "buybackAllToTreasury"],
        minOutWei,
        overrides
      );
      await onRefreshRouterInfo();
    ...
    liq_buybackAllToTreasury: async (minOut) => {
      ...
      await writeFirst(
        [getLiquidityContract],
        ["buybackAllToTreasury", "buyBiggiAndSendToTreasury", "buybackToTreasury"],
        minOutWei
      );
      await onRefreshRouterInfo();
      await onRefreshBuybackInfo();
    ...
    liq_addLiquidityFromBalances: async (biggiAmount, nativeAmount) => {
      const overrides = nativeAmount ? { value: parseEth(nativeAmount) } : {};
      const tokenWei = biggiAmount ? parseEth(biggiAmount) : ethers.constants.Zero;
      await writeFirst(
        [getLiquidityContract],
        ["addLiquidityFromBalances", "addLiquidityFromBalance", "addLiquidity"],
        tokenWei,
        overrides
      );
      await onRefreshLiquidityPreview();
    ...
    liq_bootstrapLiquidity: async (tokenAmount, nativeAmount) => {
      ...
      const overrides = nativeAmount ? { value: parseEth(nativeAmount) } : {};
      await writeFirst([getLiquidityContract], ["bootstrapLiquidity", "bootstrapLp"], tokenWei, overrides);
      await onRefreshLiquidityPreview();
    },
    liq_routeBiggiToTreasury: async (amount) => {
      const amtWei = parseEth(amount || 0);
      await writeFirst([getLiquidityContract], ["routeBiggiToTreasury", "routeTokensToTreasury", "sendBiggiToTreasury"], amtWei);
    },

    /* --- Policy tokenomics setters --- */
    pol_setSplits: async (alphaBps, betaBps, gammaBps) => {
      await writeFirst([getPolicy], ["setSplits", "setSplitBps", "setPolicySplits", "setAllocations"], Number(alphaBps), Number(betaBps), Number(gammaBps));
    },
    pol_setGuards: async (g) => {
      const dailyWei = g.dailyCapNative ? parseEth(g.dailyCapNative) : ethers.constants.Zero;
      const args = [
        Number(g.swapSlip),
        Number(g.lpSlip),
      ...
      await writeFirst([getPolicy], ["setGuards", "configureGuards", "setLimits"], ...args);
    },
    pol_setPauses: async ({ buybacks, refills, lpAdds, eoc }) => {
      await writeFirst([getPolicy], ["setPauses", "setPauseFlags", "setPaused"], !!buybacks, !!refills, !!lpAdds, !!eoc);
    },
    pol_setOperator: async (addr, allowed) => {
      if (!addr) throw new Error("Operator address is empty");
      await writeFirst([getPolicy], ["setOperator", "setOperatorAllowed", "setOperatorPermission"], addr, !!allowed);
    },
    pol_consumeDaily: async (amtNative) => {
      const wei = parseEth(amtNative || 0);
      await writeFirst([getPolicy], ["consumeDaily", "consumeDailyAllowance", "consumeDailyCap"], wei);
    },
    pol_resetDailyCounter: async () => {
      await writeFirst([getPolicy], ["resetDaily", "resetDailyCounter", "resetDailyCap"]);
    },
