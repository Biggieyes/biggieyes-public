...
export function useVRF() {
  const queryLogsBatched = async (contract, filter, fromBlock, toBlock, step = 2000) => {
    const out = [];
    let start = fromBlock;
    let batch = step;
    while (start <= toBlock) {
  ...
  const buildVRFHistory = React.useCallback(async (c, address) => {
    ...
    const fulfillLogsRaw = await queryLogsBatched(c, c.filters.VRFFulfillStarted(), from, latest);
  ...
  const refreshVRFPanel = React.useCallback(async (walletAddress) => {
    try {
  ...
  const checkVrfResolution = React.useCallback(async (walletAddress) => {
    try {
      if (!walletAddress) return { resolved: false };
      const c = getReadOnlyContract();
      const rid = await c.pendingMintRequest(walletAddress).catch(() => ethers.BigNumber.from(0));
