...
const DistributorTokenTab = ({
  ...
  const nativeTotals = React.useMemo(() => {
    ...
    const pendingCollection = toNumber(distributorData?.pendingCollectionRewards);
    const pendingCommunity = toNumber(distributorData?.pendingCommunityCenter);

    const sumPending = [pendingReserve, pendingTreasury, pendingBuyback, pendingCollection, pendingCommunity]
      .filter((val) => Number.isFinite(val))
      .reduce((acc, val) => acc + val, 0);

    const totalPendingRaw = toNumber(distributorData?.totalPending);
    const totalPending = Number.isFinite(totalPendingRaw) ? totalPendingRaw : sumPending;

    return {
      totalPending,
      totalReceived: toNumber(distributorData?.totalReceived ?? distributorData?.totalDistributed),
    };
  }, [distributorData]);

  const tokenBalances = React.useMemo(() => {
    const reserve = tokenSnapshot?.token?.balances?.reserveNumeric ?? null;
    const dripDistributor = tokenSnapshot?.token?.balances?.dripDistributorNumeric ?? null;
    const tokenRewards = tokenSnapshot?.token?.balances?.tokenRewardsNumeric ?? null;
  ...
  const tokenCapTotal = React.useMemo(() => {
    const capFromSnapshot = toNumber(tokenSnapshot?.token?.capNumeric ?? tokenSnapshot?.token?.cap);
    const totalSupplyFromSnapshot = toNumber(tokenSnapshot?.token?.totalSupplyNumeric ?? tokenSnapshot?.token?.totalSupply);
    const totalSupplyProp = toNumber(tokenTotalSupply);
    return capFromSnapshot ?? totalSupplyFromSnapshot ?? totalSupplyProp ?? tokenBalances.total;
  }, [tokenSnapshot, tokenTotalSupply, tokenBalances.total]);

  const biggiDestinations = React.useMemo(() => {
    const { total, reserve, dripDistributor, tokenRewards, treasury, buyback } = tokenBalances;
    return [
      { label: "Reserve", amount: formatAmount(reserve, "BIGGI"), share: shareOf(reserve, total) },
      { label: "Drip Distributor", amount: formatAmount(dripDistributor, "BIGGI"), share: shareOf(dripDistributor, total) },
      { label: "TokenRewards", amount: formatAmount(tokenRewards, "BIGGI"), share: shareOf(tokenRewards, total) },
      { label: "Treasury", amount: formatAmount(treasury, "BIGGI"), share: shareOf(treasury, total) },
      { label: "Buyback", amount: formatAmount(buyback, "BIGGI"), share: shareOf(buyback, total) },
    ];
  }, [tokenBalances]);

  const tokenCapDestinations = React.useMemo(() => {
    const { reserve, dripDistributor, tokenRewards, treasury, buyback } = tokenBalances;
    const capTotal = tokenCapTotal;
    return [
      { label: "Reserve (cap)", amount: formatAmount(reserve, "BIGGI"), share: shareOf(reserve, capTotal) },
      { label: "Drip Distributor (cap)", amount: formatAmount(dripDistributor, "BIGGI"), share: shareOf(dripDistributor, capTotal) },
      { label: "TokenRewards (cap)", amount: formatAmount(tokenRewards, "BIGGI"), share: shareOf(tokenRewards, capTotal) },
      { label: "Treasury (cap)", amount: formatAmount(treasury, "BIGGI"), share: shareOf(treasury, capTotal) },
      { label: "Buyback (cap)", amount: formatAmount(buyback, "BIGGI"), share: shareOf(buyback, capTotal) },
  ...
  const tokenHooks = React.useMemo(() => {
    ...
    const linkFor = (address) => (address ? `${explorerBase}/address/${address}` : null);
    return [
      { label: "BIGGI token", address: tokenSnapshot?.token?.address || ADDR.BIGGI },
      { label: "Reserve hook", address: hooks.reserve || ADDR.RESERVE },
      { label: "DripDistributor hook", address: hooks.dripDistributor || ADDR.DRIP_DISTRIBUTOR },
      { label: "TokenRewards hook", address: hooks.tokenRewards || ADDR.TOKEN_REWARDS },
    ...
    <section className="distributor-token-tab">
      ...
      <div className="distributor-token-tab__grid">
        ...
        <StatCard
          ...
          tone="native"
        />
        <StatCard label="BIGGI available to mint" value={formatMaybeAmount(dripAvailable, "BIGGI")} hint="Cap / reserve" tone="token" />
        <StatCard label="Total BIGGI supply" value={formatMaybeAmount(tokenTotalSupply, "BIGGI")} hint="Tokenomics" accent="secondary" tone="token" />
      </div>
      <div className="distributor-token-tab__content">
        <div className="distributor-token-tab__column">
          <DestinationsTable title="BIGGI token cap distribution" items={tokenCapDestinations} />
          <TokenFlow flows={biggiDestinations.map((item) => ({ label: item.label, amount: item.amount, hint: item.share }))} />
        </div>
        <div className="distributor-token-tab__column">
