...
export async function refreshTokenMeta({
  ...
  const tokenAddr =
    (await callFirst(brl, ["tokenAddress", "token", "getToken", "biggi", "biggiToken"])) ||
    ADDR.BIGGI ||
    ADDR.BIGGI_TOKEN ||
    null;

  if (!tokenAddr) throw new Error("Unable to resolve BIGGI token address. Configure ADDR.BIGGI or expose tokenAddress().");

  const meta = await callFirst(brl, ["tokenMeta", "getTokenMeta", "biggiMeta"]);
  const capLeft = await callFirst(brl, ["remainingCap", "capLeft", "getRemainingCap"]);
  ...
  setBiggiData((prev) => ({
    ...
    token: {
      ...
      collectionRewardsAddr: dist?.collectionRewards || prev.token.collectionRewardsAddr,
      communityCenterAddr: dist?.communityCenter || prev.token.communityCenterAddr,
      tokenRewardsAddr: tr?.tokenRewards || prev.token.tokenRewardsAddr || tokenRewardsAddr,
    },
    router: {
      ...prev.router,
    ...
    liquidity: {
      ...
      biggiPerNative: fmt(core?.biggiPerNative) ?? prev.liquidity.biggiPerNative,
      nativePerBiggi: fmt(core?.nativePerBiggi) ?? prev.liquidity.nativePerBiggi,
    },
    distributor: {
      ...prev.distributor,
      address: dist?.distributor || prev.distributor?.address,
      totalReceived: fmt(dist?.totalReceived) ?? prev.distributor?.totalReceived,
      pendingBuyback: fmt(dist?.pendingBuyback) ?? prev.distributor?.pendingBuyback,
      collectionRewards: dist?.collectionRewards || prev.distributor?.collectionRewards,
      ...
      communityCenter: dist?.communityCenter || prev.distributor?.communityCenter,
      pendingCommunity: communitySnapshot?.pendingCommunity ?? prev.distributor?.pendingCommunity,
      communityPoolBalance: communitySnapshot?.communityPoolBalance ?? prev.distributor?.communityPoolBalance,
    },
    buyback: {
      ...prev.buyback,
      buybackAgent: buy?.buybackAgent || dist?.buybackAgent || prev.buyback?.buybackAgent,
      ...
      totalNativeReceived: fmt(buy?.totalNativeReceived) ?? prev.buyback?.totalNativeReceived,
      totalNativeSpent: fmt(buy?.totalNativeSpent) ?? prev.buyback?.totalNativeSpent,
      totalBiggiAcquired: fmt(buy?.totalBiggiAcquired) ?? prev.buyback?.totalBiggiAcquired,
      autoBuybackEnabled: buy?.autoBuybackEnabled ?? prev.buyback?.autoBuybackEnabled,
      paused: buy?.paused ?? prev.buyback?.paused,
      lastBuybackAt: Number(buy?.lastBuybackAt || prev.buyback?.lastBuybackAt || 0),
      router: buy?.router || core?.router || prev.router.routerAddress,
      wrappedNative: buy?.wrappedNative || core?.weth || prev.router.wrappedNative,
      treasury: buy?.treasury || dist?.treasury || prev.buyback?.treasury,
    },
    reserve: {
      ...prev.reserve,
      reserveAddress: res?.reserve || prev.reserve?.reserveAddress || reserveAddr,
      ...
      lpBalanceInVault: fmt(res?.lpBalanceInVault) ?? prev.reserve?.lpBalanceInVault,
      pairWhitelisted: res?.pairWhitelisted ?? prev.reserve?.pairWhitelisted,
    },
    rewards: {
      ...prev.rewards,
      unitReward: tr?.unitReward != null ? fmt(tr.unitReward) : prev.rewards.unitReward,
      rewardsCap: tr?.rewardsCap != null ? fmt(tr.rewardsCap) : prev.rewards.rewardsCap,
      rewardsMinted: tr?.rewardsMinted != null ? fmt(tr.rewardsMinted) : prev.rewards.rewardsMinted,
      balance: tr?.balance != null ? fmt(tr.balance) : prev.rewards.balance,
      blockWeights: Array.isArray(tr?.blockWeights) ? Array.from(tr.blockWeights, (x) => Number(x)) : prev.rewards.blockWeights,
      dripAvailable: drip?.availableTokens != null ? fmt(drip.availableTokens) : prev.rewards.dripAvailable,
      dripTotalClaimed: drip?.totalClaimed != null ? fmt(drip.totalClaimed) : prev.rewards.dripTotalClaimed,
      dripTotalNotified: drip?.totalNotified != null ? fmt(drip.totalNotified) : prev.rewards.dripTotalNotified,
      dripTokensPerMint: drip?.tokensPerMint != null ? fmt(drip.tokensPerMint) : prev.rewards.dripTokensPerMint,
      dripDistributor: drip?.dripDistributor || prev.rewards.dripDistributor,
      dripLm: drip?.dripLM || drip?.dripLm || prev.rewards.dripLm,
      dripTotalTopUp: drip?.totalTopUp != null ? fmt(drip.totalTopUp) : prev.rewards.dripTotalTopUp,
