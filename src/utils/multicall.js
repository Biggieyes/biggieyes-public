...
function _getMulticallAddress() {
  try {
    if (typeof process !== 'undefined' && process.env && process.env.VITE_MULTICALL_ADDRESS) return process.env.VITE_MULTICALL_ADDRESS;
  } catch {
    // ignore env access errors
  }
  try {
    if (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_MULTICALL_ADDRESS) return import.meta.env.VITE_MULTICALL_ADDRESS;
  } catch {
    // ignore env access errors
  }
  try {
 ...
 */
export async function multicallAggregate(provider, calls = [], multicallAddress = null) {
  ...
  if (!addr) {
    ...
    const results = await Promise.all(calls.map(async (c) => {
      ...
      const res = await provider.call({ to: c.target, data }).catch((e) => { throw e; });
  ...
  const callInput = calls.map((c) => ({ target: c.target, callData: c.iface.encodeFunctionData(c.method, c.params || []) }));
  const [, returnData] = await mc.aggregate(callInput);
  return returnData.map((rd, i) => calls[i].iface.decodeFunctionResult(calls[i].method, rd));
}
